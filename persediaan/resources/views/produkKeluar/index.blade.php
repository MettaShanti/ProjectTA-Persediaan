@extends('layouts.main')

@section('content')
<h4>Daftar Produk Keluar</h4>
<a href="{{ route('produkKeluar.create') }}" class="btn btn-primary mb-3">TAMBAH</a>

<div class="table-responsive">
    <table class="table table-striped table-bordered table-hover">
        <thead class="table-dark">
            <tr>
                <th>Nama Produk</th>
                <th>Tanggal Produk Keluar</th>
                <th>Tanggal Produksi</th>
                <th>Tanggal Expiret</th>
                <th>Jumlah</th>
                <th>Aksi</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($produkKeluar as $row)
            <tr>
                <td>{{ $row['produk']['nama_produk']}}</td>
                <td>{{ $row->tgl_keluar}}</td>
                <td>{{ $row->tgl_produksi}}</td>
                <td>{{ $row->tgl_exp}}</td>
                <td>{{ $row->jumlah}}</td>
                <td>
                    <a href="{{ route('produkKeluar.edit', $row->id) }}" class="btn btn-warning btn-sm">UBAH</a>
                    <form action="{{ route('produkKeluar.destroy', $row->id) }}" method="post" style="display:inline">
                        @csrf
                        @method('DELETE')
                        <button class="btn btn-danger btn-sm" onclick="return confirm('Anda yakin ingin hapus data ini?')">HAPUS</button>
                    </form>
                </td>
            </tr>
            @endforeach
        </tbody>
    </table>
</div>
@endsection