...
<form action="{{ route('produkKeluar.update', $produkKeluar->id) }}" method="POST">
    ...
    <select name="kode_produk" id="kode_produk" class="form-control mb-2">
        ...
        @foreach($produks as $produk)
            <option value="{{ $produk->kode_produk }}"
                ...
                {{ old('kode_produk', $produkKeluar->kode_produk) == $produk->kode_produk ? 'selected' : '' }}>
                {{ $produk->kode_produk }}
            </option>
        @endforeach
    </select>
    ...
    <input type="text" name="nama_produk" id="nama_produk" class="form-control mb-2" value="{{ old('nama_produk', $produkKeluar->nama_produk) }}" readonly>

    <input type="hidden" name="produk_id" id="produk_id" value="{{ old('produk_id', $produkKeluar->produk_id) }}">

    <label>Tanggal Keluar</label>
    @error('tgl_keluar') <span class="text-danger">({{ $message }})</span> @enderror
    <input type="date" name="tgl_keluar" class="form-control mb-2" value="{{ old('tgl_keluar', $produkKeluar->tgl_keluar) }}">

    <label>Tanggal Expired</label>
    @error('tgl_exp') <span class="text-danger">({{ $message }})</span> @enderror
    <input type="date" name="tgl_exp" id="tgl_exp" class="form-control mb-2" value="{{ old('tgl_exp', $produkKeluar->tgl_exp) }}">

    <label>Jumlah</label>
    @error('jumlah') <span class="text-danger">({{ $message }})</span> @enderror
    <input type="number" name="jumlah" class="form-control mb-2" value="{{ old('jumlah', $produkKeluar->jumlah) }}">
